...
def vis_detections(im, class_name, dets, im_class, thresh=0.5):
    ...
    for i in inds:
        ...
        ax.text(bbox[2] - 10 , bbox[3] + 2, '{:s}'.format(im_class), bbox=dict(facecolor='red', alpha=0.5), fontsize=14, color='white')
...
def demo(net, image_name):
    ...
    im_class = CLASSES[int((im_file.split('/')[-1]).split('_')[0])]
    ...
    for cls_ind, cls in enumerate(CLASSES[1:]):
        ...
        #import pdb
        #pdb.set_trace()
        vis_detections(im, cls, dets, im_class, thresh=CONF_THRESH)
