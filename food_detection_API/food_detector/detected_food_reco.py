...
data_category = pd.read_csv('./food_detector/lib/datasets/category.txt', '\t')
...
def detect_object(net, image_name):
    ...
    for cls_ind, cls in enumerate(CLASSES[1:]):
        ...
        if len(inds) > 0:
            bbox = dets[inds, :4]
            score = dets[inds, -1]
            detected_obj.append({'detected_class': cls, 'boxes': (bbox[0,]).tolist(), 'score': score.tolist()[0]})
...
def reco_main(im_name):
    ...
    # args = parse_args()
    prototxt = "./food_detector/models/food_rec/test.prototxt"
    caffemodel ='./food_detector/output/food_rec/train1/zf_faster_rcnn_iter_30000.caffemodel'
    ...
    #im_names = os.listdir("./data/demo/")i
    #im_name = args.imgpath
    recommendation = dict()
    
   # im_base_name = os.path.basename(im_name)
    #im_class = CLASSES[int((im_base_name.split('_')[0]))]
    recommendation[im_name] = detect_object(net, im_name)
